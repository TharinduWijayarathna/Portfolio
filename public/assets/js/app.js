  ...
  // --------------------------------------------- //
    ...
    $(document).ready(function() {
        $("#contact-form").submit(function(event) {
            ...
            $.ajax({
                type: "POST",
                url: th.attr('action'), // Use the form's action URL
                data: th.serialize() // Serialize the form data
            }).done(function() {
                $('.form-container').find('.contact-form').addClass('is-hidden');
                $('.form-container').find('.form__reply').addClass('is-visible');
                setTimeout(function() {
                    // Done Functions
                    $('.form-container').find('.form__reply').removeClass('is-visible');
                    $('.form-container').find('.contact-form').removeClass('is-hidden');
                    th.trigger("reset");
                }, 5000);
            }).fail(function() {
